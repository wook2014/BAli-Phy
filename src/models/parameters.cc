...
int data_partition::n_base_models() const
{
    ...
    return P->evaluate(P->PC->SModels[s].n_base_models, false).as_int();
}

int data_partition::n_states() const
{
    int s = *P->smodel_index_for_partition(partition_index);
    return P->evaluate(P->PC->SModels[s].n_states, false).as_int();
}

Matrix data_partition::WeightedFrequencyMatrix() const
{
    int s = *P->smodel_index_for_partition(partition_index);
    return P->evaluate(P->PC->SModels[s].weighted_frequency_matrix, false).as_<Box<Matrix>>();
}

EVector data_partition::state_letters() const
{
    int s = *P->smodel_index_for_partition(partition_index);
    return P->evaluate(P->PC->SModels[s].state_letters, false).as_<EVector>();
}
...
{
    return P->evaluate(DPC().conditional_likelihoods_for_branch[b],false).as_<Likelihood_Cache_Branch>();
}
                                     ...
                                     bool allow_compression)
{
    ...
    program_file<<"-- Use the program `hindent` to indent this file for readability\n";
