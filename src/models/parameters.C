...
void data_partition::variable_alignment(bool b)
{
  ...
  {
    subA_ = new subA_index_leaf(this, subA_row_indices, subA_up_to_date, T().n_branches()*2);
  ...
  else 
  {
    if (use_internal_index)
      subA_ = new subA_index_internal(this, subA_row_indices, subA_up_to_date, T().n_branches()*2);
    else
      subA_ = new subA_index_leaf(this, subA_row_indices, subA_up_to_date, T().n_branches()*2);
   ...
   branch_HMM_type(T().n_branches(),0)
{
  ...
  if (variable_alignment() and use_internal_index)
    subA_ = new subA_index_internal(this, subA_row_indices, subA_up_to_date, B*2);
  else
    subA_ = new subA_index_leaf(this, subA_row_indices, subA_up_to_date, B*2);
...
void Parameters::NNI(int br1, int br2)
{
  ...
  for(int i=0;i<n_data_partitions();i++)
    if (get_data_partition(i).variable_alignment())
    {
      get_data_partition(i).set_pairwise_alignment(T().directed_branch(nodes[0],nodes[4]), get_pairwise_alignment_from_bits(a123456[i], 0, 4), true);
      get_data_partition(i).set_pairwise_alignment(T().directed_branch(nodes[1],nodes[4]), get_pairwise_alignment_from_bits(a123456[i], 1, 4), true);
      get_data_partition(i).set_pairwise_alignment(T().directed_branch(nodes[2],nodes[5]), get_pairwise_alignment_from_bits(a123456[i], 2, 5), true);
      get_data_partition(i).set_pairwise_alignment(T().directed_branch(nodes[3],nodes[5]), get_pairwise_alignment_from_bits(a123456[i], 3, 5), true);
      get_data_partition(i).set_pairwise_alignment(T().directed_branch(nodes[4],nodes[5]), get_pairwise_alignment_from_bits(a123456[i], 4, 5), true);
    }
  */
...
int Parameters::SPR(int br1, int br2, int branch_to_move)
{
  ...
  recompute_pairwise_alignment(T().directed_branch(m1,m2));
